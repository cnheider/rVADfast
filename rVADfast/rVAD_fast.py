...
def get_rvad(
    ...
    *,
    ...
    ft, flen, fsh10, nfr10 = speechproc.sflux(
        data, sample_rate, win_len, ovr_len, n_ftt
    )
...
if __name__ == "__main__":
    ...
    def main():
        in_wave_file = str(sys.argv[1])
        output_vad_label = str(sys.argv[2])

        sample_rate, data = speechproc.speech_wave(in_wave_file)

        vad_seg = get_rvad(sample_rate, data)

        numpy.savetxt(output_vad_label, vad_seg.astype(int), fmt="%i")
        print(f"{in_wave_file} --> {output_vad_label} ")
