...
def snre_highenergy(fdata,
  ...
  for i in range(2, nfr10 + 1):
    postsnr[i] = numpy.log10(e[i]) - numpy.log10(e_min[i])
    if numpy.less(postsnr[i], 0):
      postsnr[i] = 0

    d_[i] = numpy.sqrt(numpy.abs(e[i] - e[i - 1]) * postsnr[i])
  d_[1] = d_[2]
...
def snre_vad(fdata, nfr10, flen, fsh10, energy_floor, pv01, pvblk, vad_threshold, *, d_expl=18, d_expr = 18):
  ## ---*******- important *******
  # here [0] index array element has  not used
  ...
  for i in range(1, nfr10 + 1):
    ...
    if (pvblk_[i] == 1) and (sign_pv == 0):
      ...
      e_y = numpy.sort(e[range(nstart, nstop + 1)])
      e_y = numpy.insert(e_y, 0, "inf")  # as [0] is discarding

      emin = e_y[int(numpy.floor((nstop - nstart + 1) * 0.1))]
      ...
      d_[nstart] = d_[nstart + 1]

      tm1 = numpy.hstack(
          (numpy.ones(d_expl) * d_[nstart], d_[range(nstart, nstop + 1)])
          )
      d_exp = numpy.hstack((tm1, numpy.ones(d_expr) * d_[nstop]))

      d_exp = numpy.insert(d_exp, 0, "inf")

      for j in range(0, nstop - nstart + 1):
        d_smth[nstart + j] = sum(d_exp[range(j + 1, j + d_expl + d_expr + 1)])

      d_smth_threshold = sum(
          d_smth[range(nstart, nstop + 1)] * pv01_[range(nstart, nstop + 1)]
  ...
  for i in range(1, nfr10 + 1):
    if (pv_vad[i] == 1) and (sign_vad == 0):
      ...
      if i == nfr10:
        nstop = i
      sign_vad = 0
      e_sum = e_sum + sum(e[range(nstart, nstop + 1)])
...
def pitchblockdetect(pv01, pitch, nfr10, opts):
  ...
  for i in range(0, nfr10):
    ...
    if (pv01_[i] == 1) and (sign_pv == 0):
      # print("i=%s " %(i))
      nstart, sign_pv = i, 1
      pv_blk[range(max([nstart - 60, 0]), nstart + 1)] = 1
      ...
      pv_blk[range(nstop, numpy.amin([nstop + 60, nfr10 - 1]) + 1)] = 1
